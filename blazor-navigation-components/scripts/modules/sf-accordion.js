...
window.sfBlazor.Accordion = (function () {
    ...
    var SfAccordion = /** @class */ (function () {
        function SfAccordion(dataId, element, options, dotnetRef) {
            window.sfBlazor = window.sfBlazor;
            this.keyConfigs = {
                moveUp: 'uparrow',
                moveDown: 'downarrow',
        ...
        SfAccordion.prototype.expandAnimation = function (ef, icn, trgt, trgtItemEle, animate, args) {
            ...
            if (ef === 'None' && sf.base.animationMode === 'Enable') {
                ef = 'SlideDown';
        ...
        SfAccordion.prototype.expandProgress = function (progress, icon, trgt, trgtItemEle, eventArgs) {
            ...
            if (progress === 'end') {
                ...
                sf.base.attributes(trgtItemEle.firstElementChild, { 'aria-expanded': 'true' });
        ...
        SfAccordion.prototype.expandedItemsPop = function (item) {
            var index = this.getIndexByItem(item);
            var temp = [].slice.call(this.options.expandedIndices);
            var tempIndex = temp.indexOf(index);
            if (tempIndex > -1) {
        ...
        SfAccordion.prototype.collapseProgress = function (progress, icon, trgt, trgtItemEle, eventArgs) {
            ...
            if (progress === 'end') {
                ...
                sf.base.attributes(trgtItemEle.firstElementChild, { 'aria-expanded': 'false' });
        ...
        SfAccordion.prototype.expandingItem = function (expandArgs) {
            ...
            if ((animation.name === 'None' && sf.base.animationMode !== 'Enable') || sf.base.animationMode === 'Disable') {
    ...
    var Accordion = {
        initialize: function (dataId, element, options, dotnetRef) {
            if (element && dataId) {
                ...
                var instance = new SfAccordion(dataId, element, options, dotnetRef);
                instance.render();
        ...
        setExpandModeAndRTL: function (dataId, enableRtl, expandMode, isRtlChanged, isExpandModeChanged) {
            var instance = window.sfBlazor.getCompInstance(dataId);
            if (instance.element) {
                if (isRtlChanged) {
                    if (enableRtl) {
        ...
        itemChanged: function (dataId) {
            var instance = window.sfBlazor.getCompInstance(dataId);
            if (instance.element) {
                instance.wireFocusEvents();
            }
        },
        afterContentRender: function (dataId, targetEle, animation) {
            var instance = window.sfBlazor.getCompInstance(dataId);
            if (instance.element) {
        ...
        itemExpandedOrCollapsed: function (dataId, args) {
            var instance = window.sfBlazor.getCompInstance(dataId);
            if (instance.element) {
                var content = instance.accItem[args.index].querySelector('.' + CLS_CONTENT);
