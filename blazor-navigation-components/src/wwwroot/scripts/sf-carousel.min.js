(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["_content/Syncfusion.Blazor.Navigations/scripts/sf-carousel"],{
...
window.sfBlazor.Carousel = function () {
      ...
      if (e.swipeDirection === 'Left' || e.swipeDirection === 'Right') {
        ...
        if (offsetDist > time || e.distanceX > this.element.offsetWidth / 2) {
          ...
          if (!this.element.classList.contains('e-loop') && e.swipeDirection === 'Right' && selectedIndex === '0') {
            ...
            this.cancelSwipe();
          } else if (eventName && (this.swipeMode === 'Touch' && eventName.includes('Mouse') || this.swipeMode === 'Mouse' && eventName.includes('Touch'))) {
            this.cancelSwipe();
          } else {
  ...
  var BlazorCarousel = {
    ...
    updateTouch: function updateTouch(dataId, enableTouchSwipe, swipeMode) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
...
}]);(async()=>{await import(`${document.baseURI}_content/Syncfusion.Blazor.Core/scripts/syncfusion-blazor-base.min.js`).then(()=>{sfBlazor.loadDependencies('sfcarousel');})})();