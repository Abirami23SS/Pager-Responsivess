...
namespace Syncfusion.Blazor.Navigations.Internal
{
    public partial class SfMenuBase<TValue>: SfBaseComponent
    {
        ...
        internal Dictionary<string, object> AnimationSettingsObj;
        internal MenuFieldSettings Fields { get; set; } = new MenuFieldSettings();
        ...
        internal double? Top;
        internal bool EnterKey;
        private string cssClass;
        private bool enableRtl;
        private string customDataId;
        internal bool isItemClicked;

        protected string Initialize(string container, string id)
        {
            customDataId = id;
        ...
        internal ItemModel<T> BeforeItemCreation<T>(T item, int index, List<ClassCollection> itemClasses, bool triggerEvent = true)
        {
            if (itemClasses.Count == index)
            {
                ...
                if (triggerEvent)
                {
                    if (typeof(T) == typeof(MenuItemModel) && SelfRefDelegates != null)
                    {
                        InvokeAsync(async () =>
                        {
                            await SfBaseUtils.InvokeEvent(SelfRefDelegates?.OnItemRender, new MenuEventArgs<T>() { Name = BEFORERENDER, Item = item }).ConfigureAwait(true);
                        });
                    }
                    else
                    {
                        InvokeAsync(async () =>
                        {
                            await SfBaseUtils.InvokeEvent(Delegates?.OnItemRender, new MenuEventArgs<T>() { Name = BEFORERENDER, Item = item }).ConfigureAwait(true);
                        });
                    }
                }
            }
        ...
        public async Task<bool> CloseMenuAsync(int startIndex = 0, bool isKey = false, bool refresh = false, bool hamburgerMode = false)
        {
            ...
            while (i >= startIndex)
            {
                ...
                if (i != 0)
                {
                    for (var j = 1; j <= i; j++)
                    {
                        if (j == i)
                        {
                            if (MenuItems == null)
                            {
                                if (NavIdx != null && j < NavIdx.Count) // Ensure NavIdx is valid
                                {
                                    int index = NavIdx[j];

                                    if (items != null && index >= 0 && index < items.Count) // Ensure index is valid
                                    {
                            ...
                            else
                            {
                                if (NavIdx != null && j < NavIdx.Count) // Ensure NavIdx is valid
                                {
                                    ...
                                    if (menuItems != null && index >= 0 && index < menuItems.Count) // Ensure index is valid
                                    {
                                        parentMenuItem = menuItems[index];
                                    }
                                }
                            }
                        }
                        ...
                        {
                            if (NavIdx != null && itemClasses != null && j >= 0 && j < NavIdx.Count && NavIdx[j] >= 0 && NavIdx[j] < itemClasses.Count)
                            {
                        ...
                        if (MenuItems == null)
                        {
                            if (NavIdx != null && j >= 0 && j < NavIdx.Count && items != null && items.Count > 0 && NavIdx[j] >= 0 && NavIdx[j] < items.Count)
                            {
                                var menuItem = GetMenuItem(items[NavIdx[j]]);
                                items = menuItem != null ? menuItem.Items : new List<TValue>();
                            }
                        }
                        else
                        {
                ...
                if (MenuItems == null)
                {
                    var eventArgs = await TriggerBeforeOpenCloseEvent(parentItem, items, ONCLOSE, i == 0).ConfigureAwait(true);
                    cancel = eventArgs.Cancel;
                }
                else
                {
                    var eventArgs = await TriggerBeforeOpenCloseEvent(parentMenuItem, menuItems, ONCLOSE, i == 0).ConfigureAwait(true);
                    cancel = eventArgs.Cancel;
                }
                ...
                if (cancel || IsDisposed)
                {
                    ...
                    {
                        await InvokeMethod(FOCUSMENU, customDataId).ConfigureAwait(true);
                    }
                ...
                else
                {
                    ...
                    {
                        if (itemClasses != null && NavIdx != null && i >= 0 && i < NavIdx.Count && NavIdx[i] >= 0 && NavIdx[i] < itemClasses.Count && itemClasses[NavIdx[i]]?.ClassList != null)
                        {
                ...
                if (AnimationSettings != null && AnimationSettings.Effect != MenuEffect.None && IsMenu && isItemClicked)
                {
                    await InvokeMethod("sfBlazor.Menu.toggleAnimation", customDataId).ConfigureAwait(true);
                }
                else if (AnimationSettings != null && AnimationSettings.Effect != MenuEffect.None && isItemClicked && !ShowItemOnClick)
                {
                    await InvokeMethod("sfBlazor.ContextMenu.toggleAnimation", customDataId).ConfigureAwait(true);
                }
                i--;
        ...
        internal async Task OpenCloseSubMenu<T>(T item, bool isClick = false, bool isUpDownKey = false, bool isRightKey = false, SfContextMenu<TValue> contextMenu = null, bool header = false, bool isMouseOver = false)
        {
            ...
            if (!isUpDownKey && (!ShowItemOnClick || isClick))
            {
                ...
                if (subItems == null)
                {
                    await CloseMenuAsync(itemsPos).ConfigureAwait(true);
                }
                else
                {
                    ...
                    {
                        isItemClicked = true;
                        var canceled = await CloseMenuAsync(itemsPos).ConfigureAwait(true);
                        isItemClicked = false;
                        if (!canceled)
                        {
                            if (curIdx >= 0 && curIdx < items.Count && curIdx < itemClasses.Count)  // Ensure index is valid
                            {
                                var eventArgs = await TriggerBeforeOpenCloseEvent(items[curIdx], subItems, ONOPEN, isOpen: isMouseOver ).ConfigureAwait(true);
                                if (!eventArgs.Cancel)
                                {
                                    if (NavIdx != null) // Ensure NavIdx is not null
                                    {
                                        NavIdx.Add(curIdx);
                                    }

                                    if (itemClasses[curIdx] != null) // Ensure itemClasses[curIdx] is not null
                                    {
                                        ...
                                        RemoveSelection(itemClasses, isMouseOver);
                                    ...
                                    if (isRightKey)
                                    {
                                        ...
                                        if (subIdx != -1 && subIdx < itemClasses[curIdx].ClassList.Count) // Ensure valid index
                                        {
                                    ...
                                    if (contextMenu != null)
                                    {
                                        contextMenu.scrollHeight = eventArgs.ScrollHeight;
                                    }
                    ...
                    else if (header)
                    {
                        ...
                        if (!canceled)
                        {
                            if (NavIdx.Count == 1)
                            {
                                var eventArgs = await TriggerBeforeOpenCloseEvent(default, Items, ONOPEN).ConfigureAwait(true);
                                if (!eventArgs.Cancel)
                                {
                                    SetOpenEventArgs(Items, default);
                                }
                            }
                            else
                            {
                                var parentItem = GetParentItem<T>();
                                var eventArgs = await TriggerBeforeOpenCloseEvent(parentItem, items, ONOPEN).ConfigureAwait(true);
                                if (!eventArgs.Cancel)
                                {
                                    SetOpenEventArgs(items, parentItem);
                                }
                            }
                        }
                    }
        ...
        private static void RemoveSelection(List<ClassCollection> menuItemsClasses, bool isMouseOver)
        {
            if (!isMouseOver)
                return;
            foreach (var menuItemsClass in menuItemsClasses)
            {
                if (menuItemsClass.ItemClass.Contains(SELECTED, StringComparison.Ordinal))
                {
                    menuItemsClass.ItemClass = SfBaseUtils.RemoveClass(menuItemsClass.ItemClass, SELECTED);
                }
            }
        }
        ...
        private static int CheckIndex(List<ClassCollection> itemClasses, bool isDown, int index, int count)
        {
        ...
        internal async Task ClickHandler<T>(List<T> menuItems, T item, System.EventArgs e, bool isEnterKey = false, bool isUl = false, bool header = false, bool hamburgerMode = false)
        {
            ...
            if (!isEnterKey || !isUl)
            {
                if (!header)
                {
                    if (typeof(T) == typeof(MenuItemModel) && SelfRefDelegates != null)
                    {
                        await SfBaseUtils.InvokeEvent(SelfRefDelegates.ItemSelected, new MenuEventArgs<T>() { Name = SELECT, Item = item, Event = e }).ConfigureAwait(true);
                    }
                    else
                    {
                        await SfBaseUtils.InvokeEvent(Delegates?.ItemSelected, new MenuEventArgs<T>() { Name = SELECT, Item = item, Event = e }).ConfigureAwait(true);
                    }
                }
                ...
                if (Utils.GetItemProperties<List<T>, T>(item, Fields.Children) == null && !header)
                {
                    if (hamburgerMode)
                    {
                        await CloseMenuAsync(1, isEnterKey, false, hamburgerMode).ConfigureAwait(true);
                    ...
                    {
                        await CloseMenuAsync().ConfigureAwait(true);
                    }
                }
                else if (ShowItemOnClick || IsDevice || isEnterKey || header)
                {
                    await OpenCloseSubMenu(item, true, false, isEnterKey, null, header).ConfigureAwait(true);
                }
            }
        }
        ...
        internal async Task KeyActionHandler<T>(List<T> menuItems, T item, KeyboardEventArgs e, bool isUl, bool isParentMenu = false, bool hamburgerMode = false)
        {
            ...
            else if (e.Code == ESC || (e.Code == "Tab" && IsMenu) || (((EnableRtl || SyncfusionService.options.EnableRtl) ? e.Code == ARROWRIGHT : e.Code == ARROWLEFT) && (NavIdx.Count > 1 || IsMenu)))
            {
                await CloseMenuAsync(NavIdx.Count - 1, true, false, hamburgerMode).ConfigureAwait(true);
            }
            else if ((EnableRtl || SyncfusionService.options.EnableRtl) ? e.Code == ARROWLEFT : e.Code == ARROWRIGHT)
            {
                ...
                {
                    await OpenCloseSubMenu(item, true, false, true).ConfigureAwait(true);
                }
                else if (IsMenu)
                {
                    await CloseMenuAsync(NavIdx.Count - 1, true, false, hamburgerMode).ConfigureAwait(true);
            ...
            {
                EnterKey = true;
                await ClickHandler(menuItems, item, e, true, isUl, false, hamburgerMode).ConfigureAwait(true);
            }
            else if (e.Code == "Tab")
            {
                RemoveFocus(item, menuItems, ClsCollection, true);
            }
        }
        ...
        private ItemModel<T> updateItemModel<T>(ItemModel<T> itemModel, string property, object value)
        {
            if (property == Fields.Text)
            {
                itemModel.Text = (string)value;
            }
            else if (property == Fields.IconCss)
            {
                itemModel.IconCss = (string)value;
            }
            else if (property == Fields.Children)
            {
                itemModel.Items = (List<T>)value;
            }
            else if (property == Fields.Url)
            {
                itemModel.Url = (string)value;
            }
            else if (property == Fields.Separator)
            {
                itemModel.Separator = (bool)value;
            }
            else if (property == Fields.Disabled)
            {
                itemModel.Disabled = (bool)value;
            }
            else if (property == Fields.Hidden)
            {
                itemModel.Hidden = (bool)value;
            }
            else if (property == Fields.ItemId)
            {
                itemModel.Id = (string)value;
            }
            else if (property == Fields.ParentId)
            {
                itemModel.ParentId = (string)value;
            }
        ...
        internal ItemModel<T> GetMenuItem<T>(T item)
        {
            ...
            try
            {
                ...
                if (item.GetType().Name == "ExpandoObject")
                {
                    List<string> properties = new List<string>();

                    foreach (KeyValuePair<string, object> keyValuePair in ((IDictionary<string, object>)item))
                    {
                        properties.Add(keyValuePair.Key);
                    }
                    for (int i = 0; i < (((IDictionary<string, object>)item).Keys).Count; i++)
                    {
                        itemModel = updateItemModel<T>(itemModel, properties[i], ((IDictionary<string, object>)item)[properties[i]]);
                    }
                }
                else if (item.GetType().BaseType.Name == "DynamicObject")
                {
                    IEnumerable<string> dataObjectType = (item as DynamicObject).GetDynamicMemberNames();
                    foreach (string propertyName in dataObjectType)
                    {
                        itemModel = updateItemModel<T>(itemModel, propertyName, ReflectionExtension.GetValue(item, propertyName));
                    }
                }
                else
                {
                    for (int i = 0, len = ItemPropsInfo.Length; i < len; i++)
                    {
                        ...
                        itemModel = updateItemModel<T>(itemModel, prop.Name, prop.GetValue(item));
                    }
                }
        ...
        internal T GetParentItem<T>()
        {
            T currItem = default;
            List<T> currItems = Items as List<T>;
            for (int i = 1; i < NavIdx.Count; i++)
            {
                currItem = currItems[NavIdx[i]];
                currItems = (currItems[NavIdx[i]] as MenuItem).Items as List<T>;
            }
            return currItem;
        }
        ...
        protected async Task<BeforeOpenCloseMenuEventArgs<T>> TriggerBeforeOpenCloseEvent<T>(T parentItem, List<T> subItems, string name, bool isParent = false, double left = default, double top = default, string id = null, bool isOpen = false)
        {
            var eventArgs = new BeforeOpenCloseMenuEventArgs<T>() { Cancel = false, Name = name, Element = Element, Items = subItems, TargetId = id , IsOpenHover = isOpen};
            if (isParent)
            {
                eventArgs.Left = left;
                eventArgs.Top = top;
            ...
            await SfBaseUtils.InvokeEvent(name == ONOPEN ? delegates?.OnOpen : delegates?.OnClose, eventArgs).ConfigureAwait(true);
            return eventArgs;
        }
        ...
        public void UpdateChildProperties(string key, object result)
        {
            switch (key)
            {
                case ANIMATION:
                    ...
                    AnimationSettingsObj = new Dictionary<string, object>
                    {
                        { "duration", AnimationSettings.Duration },
                        { "easing", AnimationSettings.Easing },
                        { "effect", AnimationSettings.Effect.ToString() }
                    };
                    break;
                case ITEMS:
