(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["_content/Syncfusion.Blazor.Navigations/scripts/sf-stepper"],{
...
window.sfBlazor.Stepper = function () {
  ...
  var SfStepper = /** @class */function () {
    ...
    SfStepper.prototype.renderProgressBar = function () {
      if (this.element.classList.contains(HORIZSTEP)) {
        ...
        if (!stepItemContainer.classList.contains(STEPICON) && (textEle && !textEle.classList.contains('e-text') || labelEle && !labelEle.classList.contains('e-label'))) {
          ...
          var topPos = this.element.classList.contains('e-label-before') ? this.stepperItemList.offsetHeight - stepItemEle.offsetHeight / 2 - 1 : stepItemEle.offsetHeight / 2;
    ...
    SfStepper.prototype.navigationHandler = function (index) {
      ...
      if (index >= 0 && index < Itemslength) {
        index = this.liElements[parseInt(index.toString(), 10)].classList.contains(DISABLED) ? this.activeStep : index;
      }
    ...
    SfStepper.prototype.calculateProgressbarPos = function () {
      if ((this.element.classList.contains(LABELBEFORE) || this.element.classList.contains(LABELAFTER)) && !this.element.classList.contains('e-step-type-indicator') && this.liElements[this.activeStep].classList.contains(STEPICON)) {
        ...
        if (this.element.classList.contains(RTL)) {
          value = (this.progressbar.getBoundingClientRect().right - selectedEle.getBoundingClientRect().right + selectedEle.offsetWidth / 2) / this.progressbar.offsetWidth * 100;
        }
        ...
        for (var j = 0; j < this.liElements.length; j++) {
          ...
          if (j <= this.activeStep) {
            activeLiWidth += j < this.activeStep ? this.liElements[parseInt(j.toString(), 10)].offsetWidth : j === this.activeStep && j !== 0 ? this.liElements[parseInt(j.toString(), 10)].offsetWidth / 2 : 0;
          }
        }
  ...
  var Stepper = {
    ...
    updateStepperValue: function updateStepperValue(dataId, activeStep, isInteraction) {
      if (dataId) {
        window.sfBlazor.getCompInstance(dataId).navigateToStep(activeStep, isInteraction, null);
      }
    },
    updateLinear: function updateLinear(options) {
      if (options.dataId) {
        window.sfBlazor.getCompInstance(options.dataId).updateContext(options);
      }
    },
    updateStepLength: function updateStepLength(dataId, isAdd, stepCountDiff) {
    ...
    refreshProgressbar: function refreshProgressbar(dataId, activeStep) {
      if (dataId) {
        /* eslint-disable-next-line  @typescript-eslint/no-explicit-any */
