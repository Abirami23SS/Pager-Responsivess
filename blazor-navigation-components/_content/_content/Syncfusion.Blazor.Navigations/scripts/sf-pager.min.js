(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["_content/Syncfusion.Blazor.Navigations/scripts/sf-pager"],{
...
window.sfBlazor.Pager = function () {
  ...
  var SfPager = /** @class */function () {
    ...
    SfPager.prototype.windowResized = function () {
      ...
      setTimeout(function () {
        ...
        if (!sf.base.isNullOrUndefined(mfirst) && mfirst.offsetWidth === 0) {
          ...
          if (!sf.base.isNullOrUndefined(pagerMessageDiv) && numericContainerWidth + pageSizesOffsetWidth + pagerMessageDiv.offsetWidth > pagerWithoutMargin && _this.previousBrowserWidth > pagerWidth) {
      ...
      var isStyleApplied = this.element.classList.contains('e-pager') ? getComputedStyle(this.element).getPropertyValue('padding') !== '0px' : false;
      if (isStyleApplied) {
        this.resizePager();
      }
    };
    SfPager.prototype.resizePager = function () {
      ...
      for (var i = 0; i < pagerElements.length; i++) {
        ...
        if (getComputedStyle(item).display !== 'none') {
          actualWidth += item.offsetWidth + parseFloat(getComputedStyle(item).marginLeft) + parseFloat(getComputedStyle(item).marginRight);
        }
      }
      ...
      var focusedIndex = numItemsArray.findIndex(function (item) {
        return item.classList.contains('e-currentitem');
      }) + 1;
      if (!numItems.length) {
        return;
      }
      ...
      for (var i = 0; i < numItems.length; i++) {
        var item = numItems[i];
        totalWidth += item.offsetWidth + parseFloat(getComputedStyle(item).marginLeft) + parseFloat(getComputedStyle(item).marginRight);
      }
    ...
    SfPager.prototype.hideNumericItems = function (pagerContainer, numItems, hideFrom, focusedIndex, nextPageElement, previousPageElement, numToHide) {
      ...
      for (var i = 1; i <= numToHide; i++) {
        ...
        if (focusedIndex !== 1 && (tempfocusElement === hideIndex + 1 || parseInt((_b = (_a = numItems[Math.abs(hideIndex)]) === null || _a === void 0 ? void 0 : _a.getAttribute('index')) !== null && _b !== void 0 ? _b : '0', 10) === focusedIndex || parseInt((_d = (_c = numItems[numItems.length - 1]) === null || _c === void 0 ? void 0 : _c.getAttribute('index')) !== null && _d !== void 0 ? _d : '0', 10) === focusedIndex)) {
          hideIndex = 0;
    ...
    SfPager.prototype.showNumericItems = function (hiddenNumItems, pagerWidth, actualWidth, numericItemWidth, bufferWidth, focusedIndex, showFrom) {
      ...
      var lesserIndexItems = Array.from(hiddenNumItems).filter(function (item) {
        var _a;
        return parseInt((_a = item.getAttribute('index')) !== null && _a !== void 0 ? _a : '0', 10) < focusedIndex;
      }).sort(function (a, b) {
        var _a, _b;
        return parseInt((_a = b.getAttribute('index')) !== null && _a !== void 0 ? _a : '0', 10) - parseInt((_b = a.getAttribute('index')) !== null && _b !== void 0 ? _b : '0', 10);
      });
    ...
    SfPager.prototype.setPageSizeState = function () {
      ...
      var toggleElement = function toggleElement(element, isVisible) {
    ...
    SfPager.prototype.focusOutElement = function (key) {
      ...
      if (key === 'Tab') {
        ...
        var afterPagerFirstFocusableElement = allFocusableElementsInDom.filter(function (el) {
          return !!(pageElement.compareDocumentPosition(el) & Node.DOCUMENT_POSITION_FOLLOWING);
        });
    ...
    SfPager.prototype.documentKeyHandler = function (e) {
      ...
      if (e.shiftKey && e.keyCode === 9 && activeElement !== null && activeElement.classList.contains('e-pager')) {
        ...
        if (!sf.base.isNullOrUndefined(activeElement.closest('.e-grid'))) {
          /* eslint-disable @typescript-eslint/no-explicit-any */
          if (activeElement.closest('.e-grid').__eventList.events.filter(function (e) {
            return e.name === 'shiftTabNavigation';
          }).length > 0) {
  ...
  var Pager = {
    ...
    setPageSizeState: function setPageSizeState(dataId) {
      /* eslint-disable @typescript-eslint/no-explicit-any */
      var pagerInstance = window.sfBlazor.getCompInstance(dataId);
      pagerInstance.setPageSizeState();
    },
    resizeEllipsis: function resizeEllipsis(dataId) {
      var pagerInstance = window.sfBlazor.getCompInstance(dataId);
