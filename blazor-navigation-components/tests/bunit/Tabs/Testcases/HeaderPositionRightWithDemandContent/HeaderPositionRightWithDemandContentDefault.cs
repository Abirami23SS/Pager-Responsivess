...
using Syncfusion.Blazor.Tests.Tabs.Samples.HeaderPositionRightWithDemandContent.Default;
using System;
using System.Linq;
...
{
    public class HeaderPositionRightWithDemandContentDefault : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(content => content.LoadOn, ContentLoad.Demand));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(p => p.Height, "260px").Add(content => content.LoadOn, ContentLoad.Demand));
            await Task.Delay(100);
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(p => p.Width, "600px").Add(content => content.LoadOn, ContentLoad.Demand));
            await Task.Delay(100);
            var tabEle = tab.Find("." + HelperCls.Tab);
            Assert.NotNull(tabEle);
            Assert.Contains(HelperCls.VerticalTab, tabEle.ClassName);
