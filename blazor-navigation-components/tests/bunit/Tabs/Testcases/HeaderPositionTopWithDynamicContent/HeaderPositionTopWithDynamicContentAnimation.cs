...
{
    public class HeaderPositionTopWithDynamicContentAnimation : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationPrevious>()));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationNext>()));
            await Task.Delay(100);
