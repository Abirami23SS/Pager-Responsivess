...
using Syncfusion.Blazor.Tests.Tabs.Samples.HeaderPositionTopWithDynamicContent.Default;
using System;
using System.Linq;
...
{
    public class HeaderPositionTopWithDynamicContentDefault : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(p => p.Height, "260px"));
            await Task.Delay(100);
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(p => p.Width, "600px"));
            await Task.Delay(100);
            var tabEle = tab.Find("." + HelperCls.Tab);
            Assert.NotNull(tabEle);
