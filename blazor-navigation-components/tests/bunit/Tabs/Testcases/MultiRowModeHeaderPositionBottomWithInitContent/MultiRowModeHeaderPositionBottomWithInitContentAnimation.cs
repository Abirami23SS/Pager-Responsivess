...
{
    public class MultiRowModeHeaderPositionBottomWithInitContentAnimation : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.OverflowMode, OverflowMode.MultiRow).Add(content => content.HeaderPlacement, HeaderPosition.Bottom).Add(content => content.LoadOn, ContentLoad.Init).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationPrevious>()));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.OverflowMode, OverflowMode.MultiRow).Add(content => content.HeaderPlacement, HeaderPosition.Bottom).Add(content => content.LoadOn, ContentLoad.Init).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationNext>()));
            await Task.Delay(100);
