...
using Syncfusion.Blazor.Tests.Tabs.Samples.HeaderPositionRightWithDynamicContent.Default;
using System;
using System.Linq;
...
{
    public class HeaderPositionRightWithDynamicContentDefault : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Right));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(p => p.Height, "260px"));
            await Task.Delay(100);
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(p => p.Width, "600px"));
            await Task.Delay(100);
            var tabEle = tab.Find("." + HelperCls.Tab);
            Assert.NotNull(tabEle);
            Assert.Contains(HelperCls.VerticalTab, tabEle.ClassName);
