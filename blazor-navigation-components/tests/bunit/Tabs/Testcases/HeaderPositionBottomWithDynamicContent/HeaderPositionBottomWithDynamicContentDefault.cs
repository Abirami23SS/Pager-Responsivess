...
using Syncfusion.Blazor.Tests.Tabs.Samples.HeaderPositionBottomWithDynamicContent.Default;
using System;
using System.Linq;
...
{
    public class HeaderPositionBottomWithDynamicContentDefault : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Bottom));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Bottom).Add(p => p.Height, "260px"));
            await Task.Delay(100);
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Bottom).Add(p => p.Width, "600px"));
            await Task.Delay(100);
            var tabEle = tab.Find("." + HelperCls.Tab);
            Assert.NotNull(tabEle);
