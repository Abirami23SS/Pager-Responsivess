...
{
    public class MultiRowModeHeaderPositionRightWithInitContentAnimation : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.OverflowMode, OverflowMode.MultiRow).Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(content => content.LoadOn, ContentLoad.Init).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationPrevious>()));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.OverflowMode, OverflowMode.MultiRow).Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(content => content.LoadOn, ContentLoad.Init).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationNext>()));
            await Task.Delay(100);
