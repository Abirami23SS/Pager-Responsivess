...
using Syncfusion.Blazor.Tests.Tabs.Samples.HeaderPositionLeftWithDemandContent.Default;
using System;
using System.Linq;
...
{
    public class HeaderPositionLeftWithDemandContentDefault : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Left).Add(content => content.LoadOn, ContentLoad.Demand));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Left).Add(p => p.Height, "260px").Add(content => content.LoadOn, ContentLoad.Demand));
            await Task.Delay(100);
            ...
            var style = tabEle.GetAttribute("data-sf-style").Replace(" ", string.Empty);
            Assert.Contains("height:260px", style);
            Assert.Contains("width:100%", style);
            tab.SetParametersAndRender((HelperCls.Height, "300px"));
            await Task.Delay(100);
            tabEle = tab.Find("." + HelperCls.Tab);
            style = tabEle.GetAttribute("data-sf-style").Replace(" ", string.Empty);
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.HeaderPlacement, HeaderPosition.Left).Add(p => p.Width, "600px").Add(content => content.LoadOn, ContentLoad.Demand));
            await Task.Delay(100);
            var tabEle = tab.Find("." + HelperCls.Tab);
            Assert.NotNull(tabEle);
            Assert.Contains(HelperCls.VerticalTab, tabEle.ClassName);
            ...
            tabEle = tab.Find("." + HelperCls.Tab);
            style = tabEle.GetAttribute("data-sf-style").Replace(" ", string.Empty);
            Assert.Contains("width:700px", style);
        }
