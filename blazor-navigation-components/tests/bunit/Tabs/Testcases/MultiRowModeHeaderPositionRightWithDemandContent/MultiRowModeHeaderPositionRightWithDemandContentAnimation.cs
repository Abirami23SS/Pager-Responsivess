...
{
    public class MultiRowModeHeaderPositionRightWithDemandContentAnimation : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.OverflowMode, OverflowMode.MultiRow).Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(content => content.LoadOn, ContentLoad.Demand).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationPrevious>()));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.OverflowMode, OverflowMode.MultiRow).Add(content => content.HeaderPlacement, HeaderPosition.Right).Add(content => content.LoadOn, ContentLoad.Demand).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationNext>()));
            await Task.Delay(100);
