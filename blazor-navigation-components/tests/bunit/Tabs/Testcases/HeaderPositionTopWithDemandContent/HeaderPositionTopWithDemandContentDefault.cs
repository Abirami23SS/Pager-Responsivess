...
using Syncfusion.Blazor.Tests.Tabs.Samples.HeaderPositionTopWithDemandContent.Default;
using System;
using System.Linq;
...
{
    public class HeaderPositionTopWithDemandContentDefault : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.LoadOn, ContentLoad.Demand));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(p => p.Height, "260px").Add(content => content.LoadOn, ContentLoad.Demand));
            await Task.Delay(100);
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(p => p.Width, "600px").Add(content => content.LoadOn, ContentLoad.Demand));
            await Task.Delay(100);
            var tabEle = tab.Find("." + HelperCls.Tab);
            Assert.NotNull(tabEle);
