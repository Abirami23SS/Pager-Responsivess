...
{
    public class HeaderPositionTopWithDemandContentAnimation : BunitTestContext
    {
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.LoadOn, ContentLoad.Demand).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationPrevious>()));
        ...
        {
            var tab = RenderComponent<SfTab>(options => options.Add(content => content.LoadOn, ContentLoad.Demand).AddChildContent<TabAnimationSettings>(p => p.AddChildContent<TabAnimationNext>()));
            await Task.Delay(100);
