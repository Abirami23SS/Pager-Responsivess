...
using Syncfusion.Blazor.Tests.Accordion.Samples.ContentRenderMultipleExpandMode.Default;
using System;
...
{
    public class ContentRenderMultipleExpandModeDefault : BunitTestContext
    {
        ...
        {
            var cut = RenderComponent<SfAccordion>(options => options.Add(content => content.LoadOnDemand, false));
            Assert.False(cut.Instance.EnablePersistence);
            Assert.False(cut.Instance.LoadOnDemand);
            Assert.False(cut.Instance.EnableRtl);
            Assert.Equal(ExpandMode.Multiple, cut.Instance.ExpandMode);
        ...
        public void Default()
        {
            ...
            var accordionWidth = cut.Find("." + HelperCls.Accordion).GetAttribute("data-sf-style");
            Assert.Contains("100%", accordionWidth);
            var accordionHeight = cut.Find("." + HelperCls.Accordion).GetAttribute("data-sf-style");
            Assert.Contains("auto", accordionHeight);
        ...
        public async void Interactions()
        {
            var cut = RenderComponent<Interactions>();
            Assert.Equal(3, cut.FindAll("." + HelperCls.AccordionPanel).Count);
            Assert.Equal(3, cut.FindAll("." + HelperCls.AccordionContent).Count);
        ...
        public void Height()
        {
            ...
            //Change the Height
            cut.Instance.ChangeHeightAndWidth();
            cut.Render();
            accordionEle = cut.Find("." + HelperCls.Accordion);
            accordionHeight = accordionEle.GetAttribute("data-sf-style");
            expectedValue = "width: 800px;height: 400px;";
            expectedValue.MarkupMatches(accordionHeight);
            Assert.Contains("400px", accordionHeight);
        }
        ...
        public void Width()
        {
            ...
            cut.Render();
            accordionEle = cut.Find("." + HelperCls.Accordion);
            accordionWidth = accordionEle.GetAttribute("data-sf-style");
            expectedValue = "width: 800px;height: 400px;";
            expectedValue.MarkupMatches(accordionWidth);
            Assert.Contains("800px", accordionWidth);
        }
        ...
        public async void HtmlAttributes()
        {
            ...
            var accordion = cut.Find("." + HelperCls.Accordion);
            Assert.Contains("custom-class", accordion.ClassName);
            var accordionStyle = accordion.GetAttribute("data-sf-style");
            Assert.Contains("100%", accordionStyle);
            Assert.Contains("auto", accordionStyle);
            Assert.Contains("2px solid red", accordionStyle);
            var additionalAttributes = new Dictionary<string, object>
            {{"title", "Accordion Component"}}; 
            cut = RenderComponent<HtmlAttributes>(parameters => parameters.Add(p => p.HtmlAttribute, additionalAttributes));
            cut.Render();
            accordion = cut.Find("." + HelperCls.Accordion);
            Assert.Equal("Accordion Component", accordion.GetAttribute("title"));
        }
        ...
        {
            var cut = RenderComponent<ExpandedIndices>();
            cut.FindAll("br")[0].NextElementSibling.MarkupMatches("<span>ExpandItemsLength: 3</span>");
            cut.FindAll("br")[1].NextElementSibling.MarkupMatches("<span>ExpandItemsValue: 0,1,2</span>");
            var firstItemHtml = cut.Find("." + HelperCls.AccordionItem);
            Assert.Contains(HelperCls.Selected, firstItemHtml.ClassName);
            Assert.Contains(HelperCls.Active, firstItemHtml.ClassName);
            var secondItemHtml = cut.FindAll("." + HelperCls.AccordionItem)[1];
            ...
            cut.Instance.ExpandItems = new int[] { 0, 2 };
            cut.Render();
            await Task.Delay(500);
            cut.FindAll("br")[0].NextElementSibling.MarkupMatches("<span>ExpandItemsLength: 2</span>");
            cut.FindAll("br")[1].NextElementSibling.MarkupMatches("<span>ExpandItemsValue: 0,2</span>");
            Assert.Equal("true", cut.FindAll("." + HelperCls.AccordionHeader)[0].GetAttribute("aria-expanded"));
            //Assert.Equal("true", cut.FindAll("." + HelperCls.AccordionHeader)[0].GetAttribute("aria-selected"));
        ...
        public void ARIAAttributes()
        {
            ...
            Assert.Equal("false", cut.FindAll("." + HelperCls.AccordionPanel)[1].GetAttribute("aria-hidden"));
            Assert.Equal("false", cut.FindAll("." + HelperCls.AccordionPanel)[1].GetAttribute("aria-hidden"));
        }
        ...
        {
            var accordion = RenderComponent<SfAccordion>(options => options.Add(p => p.EnableRtl, true).Add(content => content.LoadOnDemand, false));
        ...
        {
            var accordion = RenderComponent<SfAccordion>(options => options.Add(p => p.EnableRtl, false).Add(content => content.LoadOnDemand, false));
            var rootEle = accordion.Find("." + HelperCls.Accordion);
