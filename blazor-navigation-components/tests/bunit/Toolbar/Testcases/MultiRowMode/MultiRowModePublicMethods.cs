...
using Syncfusion.Blazor.Tests.Toolbar.Samples.MultiRowMode.PublicMethods;
using System.Threading.Tasks;
using Xunit;

namespace Syncfusion.Blazor.Tests.Toolbar
{
    public class MultiRowModePublicMethods : BunitTestContext
    {
        ...
        [Fact(Timeout = 10000, DisplayName = "Remove with Items property testing")]
        public async Task RemoveWithItems()
        {
            var toolbar = RenderComponent<RemoveWithItems>();
            var toolbarHtml = toolbar.Find("." + HelperCls.Toolbar);
            Assert.NotNull(toolbarHtml);
            Assert.Contains(HelperCls.Control, toolbarHtml.ClassName);
            Assert.Contains(HelperCls.Toolbar, toolbarHtml.ClassName);
        ...
        [Fact(Timeout = 10000, DisplayName = "Disable method testing")]
        public async Task DisableMethod()
        {
            var toolbar = RenderComponent<DisableMethod>();
            var toolbarHtml = toolbar.Find("." + HelperCls.Toolbar);
            Assert.NotNull(toolbarHtml);
            Assert.Contains(HelperCls.Control, toolbarHtml.ClassName);
            Assert.Contains(HelperCls.Toolbar, toolbarHtml.ClassName);
            ...
            var toolbarItems = toolbar.FindAll("." + HelperCls.ToolbarItem);
            Assert.Contains("e-overlay", toolbarItems[1].ClassName);
        }

        [Fact(Timeout = 10000, DisplayName = "Enable method testing")]
        public async Task EnableItems()
        {
            var toolbar = RenderComponent<EnableItems>();
            var toolbarHtml = toolbar.Find("." + HelperCls.Toolbar);
            Assert.NotNull(toolbarHtml);
            Assert.Contains(HelperCls.Control, toolbarHtml.ClassName);
            Assert.Contains(HelperCls.Toolbar, toolbarHtml.ClassName);
